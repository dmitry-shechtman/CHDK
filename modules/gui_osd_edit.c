...
//-------------------------------------------------------------------
// support for fonts other than 8x16
...
static OSD_elem osd[]={
    ...
    {LANG_OSD_LAYOUT_EDITOR_USB_INFO,   &conf.usb_info_pos,     {SCX(31), SCY(14)},             &conf.usb_info_scale    },  
    {LANG_OSD_LAYOUT_EDITOR_EV,         &conf.mode_ev_pos,      {12*FONT_WIDTH, FONT_HEIGHT},   &conf.mode_ev_scale     },
    {LANG_OSD_LAYOUT_EDITOR_EV_VIDEO,   &conf.ev_video_pos,     {SCX(70), SCY(24)},             &conf.ev_video_scale    },
    ...
    if (osd_to_draw)
    {
...
#if (FONT_HEIGHT <= FONT_REAL_HEIGHT)
        ...
        for (i=1; i<=2; ++i)
        {
            draw_rectangle((osd[curr_item].pos->x>=i)?osd[curr_item].pos->x-i:0, (osd[curr_item].pos->y>=i)?osd[curr_item].pos->y-i:0,
                           (osd[curr_item].pos->x+SCL(osd[curr_item].size.x,xscale))+i-1,
                           (osd[curr_item].pos->y+SCL(osd[curr_item].size.y,yscale))+i-1,
...
void osd_font_scale(int dir)   // -1=smaller only, 0=grow & wrap,  1=larger only
{
    ...
    switch ((xscale<<4)+yscale)
    {
        ...
        case 0x01 :
        case 0x10 :
            if ( dir>=0) { xscale=1; yscale=1; } else { xscale=0; yscale=0; }
            break ;
#endif
        case 0x11 :
            if ( dir>=0) { xscale=2; yscale=1 ; }
#if (FONT_HEIGHT > FONT_REAL_HEIGHT)
            else { xscale=0; yscale=0; }
#endif
            break ;
        case 0x21 :
...
#if (FONT_HEIGHT > FONT_REAL_HEIGHT)
            else if ( dir ==  0) { xscale=0; yscale=0; }
#else
...
int gui_osd_kbd_process()
{
    ...
    case KEY_LEFT:
        ...
        break;
    case KEY_RIGHT:
        if (osd[curr_item].pos->x < camera_screen.width-SCL(osd[curr_item].size.x,0))
        {
            osd[curr_item].pos->x+=(camera_screen.width-SCL(osd[curr_item].size.x,0)-osd[curr_item].pos->x>step)?step:camera_screen.width-SCL(osd[curr_item].size.x,0)-osd[curr_item].pos->x;
            osd_to_draw = 1;
        } else
            osd[curr_item].pos->x = camera_screen.width-SCL(osd[curr_item].size.x,0);
        break;
    case KEY_UP:
